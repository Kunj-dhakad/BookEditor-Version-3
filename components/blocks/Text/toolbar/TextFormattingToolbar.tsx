...
import { AlignLeft, AlignCenter, AlignRight, AlignJustify, ListChecks } from "lucide-react";
...
import CopyStyleTopAction from "@/components/HomeLayout/EditorCanvas/toolbar/EditTool/ComanEditTool/CopyStyleTopAction";
...
type TextListType = NonNullable<TextData["listType"]>;
...
const LIST_CYCLE: TextListType[] = ["none", "ul", "ol", "checklist"];

const getNextListType = (current: TextListType): TextListType => {
  const idx = LIST_CYCLE.indexOf(current);
  return LIST_CYCLE[(idx + 1) % LIST_CYCLE.length];
};
...
const getListTypeFromHTML = (html: string | undefined, fallback?: TextListType): TextListType => {
  if (fallback && fallback !== "none") return fallback;
  if (!html) return "none";
  const temp = document.createElement("div");
  temp.innerHTML = html;
  const list = temp.querySelector("ul, ol") as HTMLElement | null;
  if (!list) return "none";
  if (list.classList.contains("kd-checklist") || list.getAttribute("data-list-type") === "checklist") {
    return "checklist";
  }
  return list.tagName.toLowerCase() === "ol" ? "ol" : "ul";
};

const getTextDataListType = (data: TextData): TextListType =>
  getListTypeFromHTML(data.html, data.listType);

const getSelectionListType = (items: TextData[]): TextListType => {
  if (!items.length) return "none";
  const types = items.map(getTextDataListType);
  const first = types[0];
  return types.every((type) => type === first) ? first : "none";
};

const createListElement = (type: Exclude<TextListType, "none">, lines: string[]): HTMLElement => {
  const list = document.createElement(type === "ol" ? "ol" : "ul");
  if (type === "checklist") {
    list.className = "kd-checklist";
    list.setAttribute("data-list-type", "checklist");
  }
  lines.forEach((line) => {
...
const formatTextHTMLAsList = (
  html: string | undefined,
  text: string | undefined,
  type: TextListType
): { html: string; text: string } => {
  const sourceHTML = html || text || "";
  const temp = document.createElement("div");
  temp.innerHTML = sourceHTML;
  const existingList = temp.querySelector("ul, ol") as HTMLElement | null;

  if (type === "none") {
    if (existingList) {
      const lines = Array.from(existingList.querySelectorAll("li")).map((li) => li.innerHTML);
      const nextHTML = lines.join("<br>");
      return { html: nextHTML, text: getTextFromHTML(nextHTML) };
    }
    return { html: sourceHTML, text: getTextFromHTML(sourceHTML) };
  }
  ...
  if (existingList) {
    ...
    if (type === "checklist") {
      newList.className = "kd-checklist";
      newList.setAttribute("data-list-type", "checklist");
    }
    existingList.replaceWith(newList);
    return { html: temp.innerHTML, text: getTextFromHTML(temp.innerHTML) };
  }

  const plainText = getTextFromHTML(sourceHTML);
  const lines = plainText.split("\n").map((l) => l.trim()).filter((l) => l.length > 0);
  const list = createListElement(type, lines.length ? lines : ["List item"]);
  temp.innerHTML = "";
  temp.appendChild(list);
  return { html: temp.innerHTML, text: getTextFromHTML(temp.innerHTML) };
};

function execListOnElement(el: HTMLDivElement, type: TextListType, savedRange: Range | null): void {
  if (type === "none") {
  ...
  if (savedRange && !savedRange.collapsed) {
    ...
    if (existingList) {
      const existingType = getListTypeFromHTML(existingList.outerHTML);
    ...
    const newList = createListElement(type, lines.map((line) => line.trim()));
  ...
  if (lines.length === 0) {
    el.innerHTML = createListElement(type, ["List item"]).outerHTML;
    return;
  }

  const newList = createListElement(type, lines);
...
const TextFormattingToolbar: React.FC = () => {
  ...
  const [activeFormats, setActiveFormats] = useState({
    ...
    subscript: false, superscript: false, orderedList: false, unorderedList: false, checklist: false,
  });
  ...
  const selectedTextElements = (slides[activeSlide]?.elements || [])
    .filter((el) => selectedElementIds.includes(el.id) && el.data.type === "text");
  const selectionListType = getSelectionListType(selectedTextElements.map((el) => el.data as TextData));
  ...
  const refreshFormats = useCallback(() => {
    ...
    setActiveFormats({
      ...
      unorderedList: hasAncestor("ul") && !el.closest('.kd-checklist, [data-list-type="checklist"]'),
      checklist: !!el.closest('.kd-checklist, [data-list-type="checklist"]'),
    });
  }, []);
  ...
  const execListCycle = () => {
    ...
    const verifiedRange = el && savedRange && el.contains(savedRange.commonAncestorContainer)
      ? savedRange
      : null;

    const selectedTexts = selectedTextElements.length
    ...
    const nextListType = getNextListType(getSelectionListType(selectedTexts.map((item) => item.data as TextData)));
    pushToHistory();

    selectedTexts.forEach((item) => {
      const data = item.data as TextData;
      const activeDomMatches = item.id === activeElementId && el?.getAttribute("data-element-id") === item.id;

      if (activeDomMatches && el) {
        execListOnElement(el, nextListType, verifiedRange);
      ...
      const formatted = formatTextHTMLAsList(data.html, data.text, nextListType);
      updateElement(item.id, {
        html: formatted.html,
        text: formatted.text,
        listType: nextListType,
  ...
  const getListIcon = (listType: TextListType) => {
    switch (listType) {
      case "ol":
        return <KdListDigitIcon />;
      case "checklist":
        return <ListChecks size={16} />;
      case "ul":
      case "none":
      default:
        return < KdListItemsIcon />;
    }
  };

  const getListTooltip = (listType: TextListType) => {
    switch (listType) {
      case "ol":
  ...
  return (
    <div data-copy-style-toolbar="true" className="kd-canvasheader-container h-10 mt-4 flex items-center justify-between px-1 py-1 rounded-lg">
      ...
      <div className="flex items-center gap-1">
        ...
        {/* Single list control: None -> Bullet -> Number -> Checklist -> None. */}
        <button
          className={`kd-tooltip-parent ${iconBtn} cursor-pointer kd-canvasheader-button-all ${selectionListType !== "none" || activeFormats.unorderedList || activeFormats.orderedList || activeFormats.checklist ? "kd-icon-btn-main-active" : ""}`}
          onMouseEnter={() => setShowTooltip("list")}
          onMouseLeave={() => setShowTooltip(null)}
          onMouseDown={(e) => e.preventDefault()}
          onClick={() => { execListCycle(); setShowTooltip(null); }}
        >
          {getListIcon(selectionListType)}
          {showTooltip === "list" && <span className="kd-tooltip-bottom">{getListTooltip(selectionListType)}</span>}
        </button>
