...
/** Continuous top-to-bottom scroll, one page snapped per screen. */
export default function VerticalReader() {
  const { pages, currentPage, setCurrentPage, theme, zoomLevel } =
    useBookPreview();
  const viewportRef = useRef<HTMLDivElement>(null);
  const [scale, setScale] = useState(1);
  // Guards the two-way sync: a scroll-driven page change must not scroll back.
  ...
  useEffect(() => {
    ...
    document
      .getElementById(pageAnchorId("vertical", currentPage))
  ...
  useEffect(() => {
    ...
    pages.forEach((_, index) => {
      const element = document.getElementById(pageAnchorId("vertical", index + 1));
  ...
  return (
    <div
      ref={viewportRef}
      className="flex h-full w-full snap-y snap-mandatory select-none flex-col items-center overflow-y-auto bg-slate-200 [scrollbar-width:none]"
    >
      {pages.map((page, index) => {
        ...
        return (
          <section
            key={page.id}
            id={pageAnchorId("vertical", pageNumber)}
            data-page-number={pageNumber}
            className="relative flex h-full min-h-full min-w-full shrink-0 snap-start items-center justify-center"
          >
            <div
